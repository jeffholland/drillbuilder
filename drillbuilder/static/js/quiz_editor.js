...
async function handleImageUpload(fileInput, previewId, urlInputId) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      preview.innerHTML = `<img src="${data.image_url}" style="max-width:100px;max-height:100px;border:1px solid #ddd;border-radius:4px;" /> <button type="button" onclick="removeImage('${urlInputId}', '${previewId}')" style="font-size:11px;padding:2px 6px;">✕</button>`;
    } else {
...
function editQuestion(questionId) {
  ...
  if (question.type === 'multiple_choice') {
    ...
    if (question.prompt_image_url) {
      document.getElementById('mcqQuestionImagePreview').innerHTML = `<img src="${question.prompt_image_url}" style="max-width:100px;max-height:100px;border:1px solid #ddd;border-radius:4px;" /> <button type="button" onclick="removeImage('mcqQuestionImageUrl', 'mcqQuestionImagePreview')" style="font-size:11px;padding:2px 6px;">✕</button>`;
    }
    ...
    question.options.forEach(opt => {
      ...
      div.innerHTML = `
        ...
        <button type="button" class="choice-image-btn" onclick="triggerImageUpload(this.parentElement.querySelector('.choice-image-input'))" style="font-size:11px;padding:2px 6px;margin-left:8px;">📷</button>
    ...
    if (question.prompt_image_url) {
      document.getElementById('clozeQuestionImagePreview').innerHTML = `<img src="${question.prompt_image_url}" style="max-width:100px;max-height:100px;border:1px solid #ddd;border-radius:4px;" /> <button type="button" onclick="removeImage('clozeQuestionImageUrl', 'clozeQuestionImagePreview')" style="font-size:11px;padding:2px 6px;">✕</button>`;
    }
...
document.getElementById('addWordPair').onclick = () => {
  ...
  div.innerHTML = `
    ...
    <button type="button" onclick="triggerWMImageUpload(this, 'left')" style="font-size:11px;padding:2px 6px;margin:0 5px;">📷</button>
    <span> → </span>
    <input type="text" placeholder="Right word" class="wm-right" />
    <button type="button" onclick="triggerWMImageUpload(this, 'right')" style="font-size:11px;padding:2px 6px;margin:0 5px;">📷</button>
  ...
  if (r.status === 201) {
    ...
    document.getElementById('wordMatchPairs').innerHTML = `
      <div class="word-match-pair">
        <input type="text" placeholder="Left word" class="wm-left" />
        <button class="wm-left-image-btn" style="font-size:11px;padding:2px 4px;margin-left:4px" onclick="triggerWMImageUpload(this, 'left')">📷</button>
        <input type="file" class="wm-left-image-input" accept=".jpg,.jpeg" style="display:none" onchange="handleWMImageUpload(this, 'left')" />
        <input type="hidden" class="wm-left-image-url" />
        <div class="wm-left-image-preview" style="display:inline-block;margin-left:4px"></div>
        <span> → </span>
        <input type="text" placeholder="Right word" class="wm-right" />
        <button class="wm-right-image-btn" style="font-size:11px;padding:2px 4px;margin-left:4px" onclick="triggerWMImageUpload(this, 'right')">📷</button>
        <input type="file" class="wm-right-image-input" accept=".jpg,.jpeg" style="display:none" onchange="handleWMImageUpload(this, 'right')" />
        <input type="hidden" class="wm-right-image-url" />
